...
for the DSRec training and evaluation pipeline.
"""
...
def get_main_arguments(parser):
    ...
    parser.add_argument("--model_name", 
                        default='sasrec',
                        choices=['bert4rec', 'gru4rec', 'sasrec',   # base model
                        "dsrec_sasrec", "dsrec_bert4rec", "dsrec_gru4rec",
                        ],
                        type=str, 
                        required=False,
                        help="model name")
    parser.add_argument("--dataset", 
                        default="yelp", 
                        choices=["yelp", "fashion", "beauty", "trip", "grocery",   # preprocess by myself
                                ], 
    ...
    parser.add_argument("--threshold_user",
                        type=int,
                        default=10,
                        help="the threshold to split the short and long seq")
    parser.add_argument("--threshold_item",
                        type=int,
                        default=20,
...
def get_model_arguments(parser):
    ...
    parser.add_argument("--linear_dim",
                        default=64,
                        type=int,
                        help="the linear dimension of embedding")
    ...
    parser.add_argument("--cl_scale",
                        type=float,
                        default=0.1,
                        help="the scale for contrastive loss")
    ...
    parser.add_argument("--freeze_emb",
                        default=False,
                        action="store_true",
                        help="whether freeze the embedding layer, mainly for semantic embedding")
    ...
    parser.add_argument("--beta",
                        default=0.0,
                        type=float,
                        help="the frequency-aware weight (replaces freq_beta)")
    parser.add_argument("--sem_emb",
                        default="semantics_embeddings",
                        type=str,
                        help="the file name of the semantic embedding (without .pkl)")
...
def build_parser():
    """Build argument parser with all configuration options."""
    parser = argparse.ArgumentParser(description='DSRec: Dual-Space Sequential Recommendation')
