...
conduct early stopping according to validation performance.
"""
import numpy as np
import torch
import os
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            ...
            if callable(logger_like):
                logger_like(f'The best score is ({self.best_score:.6f} --> {score:.6f}).  Saving model ...')
            elif callable(self.trace_func):
                self.trace_func(f'The best score is ({self.best_score:.6f} --> {score:.6f}).  Saving model ...')
...
class EarlyStoppingNew():
    ...
    def save_checkpoint(self, val_loss, model, optimizer, scheduler, epoch):
        '''Saves model when validation loss decrease.'''
        if self.verbose:
            # support both standard print and logging.Logger
            logger_like = getattr(self.trace_func, 'info', None)
            if callable(logger_like):
                logger_like(f'The best score is ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
            elif callable(self.trace_func):
